...
export default function SearchPage() {
  ...
  const handleDeleteSavedSearch = async (id: string) => {
    if (!confirm("저장된 검색을 삭제하시겠습니까?")) return;
    try {
              ...
              <>
                <div className="flex items-center justify-between mb-4">
                  <h2 className="font-semibold">
                    검색 결과{" "}
                    <span className="text-blue-600">({results.total}개)</span>
                  </h2>
                  {/* 결과 요약 */}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4" onClick={() => setShowSaveModal(false)}>
          <div className="bg-white rounded-lg shadow-xl max-w-sm w-full p-5" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => setShowSaveModal(false)} className="px-3 py-1.5 text-sm text-gray-600 hover:bg-gray-100 rounded-md">취소</button>
              <button onClick={handleSaveSearch} className="px-3 py-1.5 text-sm bg-blue-600 text-white rounded-md hover:bg-blue-700">저장</button>
            </div>
          </div>
        </div>
      )}
